...
class DrawUtils {
    ...
    fun cropShapeFaces(
        ...
        // ukuran tambahan jika kotak wajah lebih besar
        val faceBox = curFace.boundingBox
        var ext = ((faceBox.right-faceBox.left)/400)*2
        if (ext == 0) {ext = 1}
        // mendapatkan titik kotak untuk wajah dan merubah ukuran
        //val faceRect = resizeRect(curFace.boundingBox, size)
        val faceRect = getRectBound(bitmap, resizeRect(curFace.boundingBox, size * ext))
    ...
    fun drawShapeFaces(
        ...
        for (face in faces) {
            ...
            var ext = ((faceBox.right-faceBox.left)/400)*2
            if (ext == 0) {ext = 1}
            val faceRect = getRectBound(bitmap, resizeRect(faceBox, size * ext))
            if (shape) {
                // gambar kotak disekitar area wajah
                tempCanvas.drawRect(faceRect, strokeLine)
            } else {
                // gambar lingkaran disekitar area wajah
    ...
    fun getSelectedFaces(faces: MutableList<Face>, select: MutableList<Int>): MutableList<Face> {
        if (faces.size == select.sum()) { return faces }

        val selectFaces = mutableListOf<Face>()
        for ((n, cond) in select.withIndex()) {
            if (cond == 1) {
                selectFaces.add(faces[n])
            }
        }
        return selectFaces
    }
}
//    fun coloredText(textView: TextView, textList: List<Pair<String, Int>>){
//        var n = 0
//        for (text in textList) {
//            val word = SpannableString(text.first)
//            word.setSpan(ForegroundColorSpan(text.second), 0, text.first.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
//            if (n == 0) {
//                textView.text = word
//                n = 1
//            } else {
